...
perform_MF_methods <- function(X, true_l, true_f, method, rankK = NULL, bnm = NULL, readOnly = F){
  ...
  ### NBSFA
  if(method == 'NBSFA') {
    print(paste0('NBSFA_results/X_tau',tau,'_K', seed, '.mat'))
    nbsfa = readMat(paste0('NBSFA_results/X_tau',tau,'_K', seed, '.mat'))
    print('readin')
    l_hat = nbsfa$L
    f_hat = nbsfa$F
  }
  ...
  if(grepl('flashr', method)){
    load(paste0('simulation/flashr_local_results/',bnm, '_',method,'.RData'))
  }else{
