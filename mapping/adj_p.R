...
FM_fn = args[1]
r2 = args[2]
FDR_alpha = as.numeric(args[3])

prefix = paste0('_LD', r2)

LLdir = '/work-zfs/abattle4/heyuan/tissue_spec_eQTL_v8/LL/'

### read in 
FM_fn = gsub('.mat', '', FM_fn)
pV_fn = paste0(LLdir, FM_fn, prefix, '_Loadings_pvalue.txt')
bT_fn = paste0(LLdir, FM_fn, prefix, '_Loadings_beta.txt')
...
### save results
pV_fn = paste0(LLdir, FM_fn, prefix, '_Loadings_pvalue_BH.txt')
bT_fn = paste0(LLdir, FM_fn, prefix, '_Loadings_beta_BH_alpha', FDR_alpha,'.txt')
