...
def find_asscio_pairs_for_snps_in_TFBS_compare_rr(ts_pair, background_pair, TF_genes, TF_snps):
	...
	print(len(ts_pair), len(set(ts_pair['Gene'])))
...
def collect_results(pair_set1, pair_set2):
    ...
    OR1, PV1, OR2, PV2, TF = [], [], [], [], []
    N1, N2, N3, N4, N5, N6 = [], [], [], [], [], []
    for tfi in np.sort(list(TF_allSNP.keys()))[IDX: IDX+1]:
...
if __name__ == '__main__':
    ...
    try:
	tp = pd.read_csv('%s/active_regions_features/per_TF/TF_enrichment_%s_THR%d%s%s_TFBS_%s_torandom_idx%d_TF%d.txt' % (enrichment_dir, featureName, THR, randomPattern, theTis, FMfn, idx, IDX), sep = '\t')
    	sys.exit()
    except:
