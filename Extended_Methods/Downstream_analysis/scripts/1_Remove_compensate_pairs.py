...
def catch_compensate_loadings_one_pair(y, w, b):
    ...
    for i in range(N):
        ...
        z_each = np.dot(b_each, X)
        z_each[np.isnan(z_each)] = 0

        compare_tissues = np.where(np.abs(z_each) > 1)[0]
        z_each = z_each[compare_tissues]
        yw       = (y*w)[compare_tissues]

        compensate = np.zeros(len(compare_tissues))
        for j in range(len(compare_tissues)):
            if (np.sign(z_each[j]) * np.sign(yw[j]) == -1):
                compensate[j] = 1
            elif (np.sign(z_each[j]) * np.sign(yw[j]) == 1) and (np.abs(yw[j]) < 3):
                compensate[j] = 1
