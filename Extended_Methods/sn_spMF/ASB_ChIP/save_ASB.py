...
def get_ASB_ratio(save_read_counts_fn, reads_filter = 10):
	...
	asb_ratio = reads_count.groupby('SNP').apply(lambda x: float(x['reads_count'].iloc[0])/np.sum(x['reads_count']))
	all_asb['ASB_ratio'] = asb_ratio[all_asb.index]
...
if __name__ == '__main__':
	...
	asb_fn.columns = ['ASB', 'ASB_ratio', 'fn', 'SNP']
	asb_fn['total_count'] = reads_count['reads_count']


	which_fn = asb_fn.groupby('SNP').apply(lambda x: x['fn'][np.argmax(np.array(x['total_count']))])
	which_fn = pd.DataFrame(which_fn)

	df = asb_fn.merge(which_fn, left_on = 'SNP', right_on = 'SNP')
	df.columns = ['ASB', 'ASB_ratio','fn', 'SNP', 'total_count', 'which_fn']
	...
	df.to_csv('%s/%s_merged_ASB.txt' % (outdir, outfn_pattern), sep='\t', index = False)
