...
#./STAR  --runMode genomeGenerate --runThreadN 12  --genomeDir ${genome_index_idr} --genomeFastaFiles ${genome_fa_dir} --sjdbGTFfile ${gtf_dir} --limitGenomeGenerateRAM 115999096192 
...
fn_rep1=${fastq_dir}/test.fastq
outputdir=/work-zfs/abattle4/heyuan/tissue_spec_eQTL_v8/datasets/TFBS_ChIP_seq/STAR_output
outputfn=${outputdir}/liver_HNF4A_ChIP_seq


#./STAR --genomeDir ${genome_index_idr}  --runThreadN 2 --readFilesIn ${fn_rep1},${fn_rep2} --outFileNamePrefix ${outputfn} --varVCFfile ${vcfFn} --outSAMattributes vA --outSAMtype BAM Unsorted --waspOutputMode SAMtag


outputfn=${outputdir}/liver_HNF4A_ChIP_seq_noWASP
./STAR --genomeDir ${genome_index_idr}  --runThreadN 2 --readFilesIn ${fn_rep1} --outFileNamePrefix ${outputfn} --varVCFfile ${vcfFn} --outSAMattributes NH HI AS nM NM MD vA vG vW --outSAMtype BAM SortedByCoordinate --waspOutputMode SAMtag
