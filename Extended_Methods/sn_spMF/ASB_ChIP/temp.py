	...
	for reads_for_one_fn in factor_reads_pool.groupby('fn'):
		...
		random_fold = 2
			...
			logreadsi = np.log10(readsi)
        		pool_i = variants_pool_asb[np.abs(variants_pool_asb['log_total_count'] - logreadsi ) < 0.01]

        		if len(pool_i) < variant_to_match_number:
            			pool_i = variants_pool_asb[np.abs(variants_pool_asb['log_total_count'] - logreadsi) < 0.1]
        		if len(pool_i) < variant_to_match_number:
            			pool_i = variants_pool_asb[np.abs(variants_pool_asb['log_total_count'] - logreadsi) < 0.2]
			if len(pool_i) < variant_to_match_number:
				pool_i = variants_pool_asb[np.abs(variants_pool_asb['log_total_count'] - logreadsi) < 0.5]
...
if __name__ == '__main__':
	...
	for idx in range(len(files)):
		fn = files[idx]
		[abs_fn_i, reads_count_i]  = get_ASB_ratio(fn)
		reads_count_i['fn'] = idx
		abs_fn_i['fn'] = idx

		abs_fn = abs_fn.append(abs_fn_i)
		reads_count = reads_count.append(reads_count_i)

	abs_fn['SNP'] = abs_fn.index
	abs_fn.columns = ['ASB', 'fn', 'SNP']
	abs_fn['total_count'] = reads_count['reads_count']


	which_fn = abs_fn.groupby('SNP').apply(lambda x: x['fn'][np.argmin(np.array(x['ASB']))])
	which_fn = pd.DataFrame(which_fn)

	df = abs_fn.merge(which_fn, left_on = 'SNP', right_on = 'SNP')
	...
	print("Number of variants with ASB:", len(abs_01), len(abs_005))


	group, factor, result_all = [], [], []
	for g in groupList:
		print(g)
		for abs_variants_list in [abs_01, abs_005]:
			[pvv1, pvv2] = compare_to_random(g, abs_variants_list)
