...
def factorsActive(featureName, sub_feature = None):
	...
	#for group in range(3):
		...
		outlier_rd = pd.read_csv('%s/%s_outlierSNPs_random_matched_%s_group%d_5folds.txt' % (sigSNPfeaturedir, FMfn, featureName, group), sep='\t', index_col = 0)
		...
		if intersectWithOpen:
			try:
				...
				continue
			outlier_rd_open = pd.read_csv('%s/%s_outlierSNPs_random_matched_%s_group%d_5folds.txt' % (sigSNPfeaturedir, FMfn, 'DNase', group), sep='\t', index_col = 0)
...
def wrap_up(featureName, sub):
	...
	background = 'random'
        if intersectWithOpen:
                enrichment_results.to_csv('%s/%s_Active_enrichment_%s_%s_to%s_WithInOpen.txt' % (chromHMM_dir, FMfn, featureName, sub.replace('/','_'), background), index=False)
                tested_N.to_csv('%s/%s_Active_enrichment_N_%s_%s_to%s_WithInOpen.txt' % (chromHMM_dir, FMfn, featureName, sub.replace('/','_'), background), index=False)
        else:
                enrichment_results.to_csv('%s/%s_Active_enrichment_%s_%s_to%s.txt' % (chromHMM_dir, FMfn, featureName, sub.replace('/','_'), background), index=False)
                tested_N.to_csv('%s/%s_Active_enrichment_N_%s_%s_to%s.txt' % (chromHMM_dir, FMfn, featureName, sub.replace('/','_'), background), index=False)
