...
def readin_enrichment(LMfn, featureName, folds = 100):
    ...
    for theTis in [-1] + range(len(Comp_tissues)):
        try:
            fntemps = [x for x in os.listdir(enrich_dir) if ((LMfn in x) and (featureName in x) and ('_%d_' % theTis in x) and ('NULL' not in x))]
	    np.random.seed(1)
...
if __name__ == '__main__':
	...
	LMfn_method = 'Thresholding_refined'
	folds = 2
