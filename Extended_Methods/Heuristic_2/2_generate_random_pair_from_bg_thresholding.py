...
def match_for_randomPairs(group, randomBatch, allGenes):
    pair_Fn = '%s/match_random/%s_outlierPairs_group%s_annotated.txt' % (pairdir, LMfn, group)
    ...
    output_Fn = open('%s/match_random/batches_random/%s_outlierPairs_random_matched_group%s_idx%d.txt' % (pairdir, LMfn, group, randomBatch), 'w')
	...
	allGenes = allGenes[allGenes!=gi]
	    ...
	    true_sets = Counter([':'.join((str(x[0]), str(x[1]))) for x in snps_for_this_gene])
	    pool_sets = Counter([':'.join((str(x[0]),str(x[1]))) for x in gi_dat.index])
...
if __name__ == '__main__':
	...
	idx = 1 * int(sys.argv[2]) #0 --> 0 to 49
        for k in range(1):
                try:
			output_Fn = pd.read_csv('%s/match_random/batches_random/%s_outlierPairs_random_matched_group%s_idx%d.txt' % (pairdir, LMfn, group, idx), sep='\t', nrows = 10, header =None)
                except:
                        match_for_randomPairs(group, idx, allGenes)
                idx += 1
