...
def each_tis_specific(tis, folds_change_to_top = None):
...
if __name__ == '__main__':
	...
	df_tissues = {}
	for tis in tissues:
    		df_tissues[tis] = each_tis_specific(tis)
		...
		shared_eQTLs.append(tis_snps_pairs)

	shared_eQTLs = [a for b in shared_eQTLs for a in b]
	shared_eQTLs = pd.DataFrame.from_dict(Counter(shared_eQTLs), orient = 'index')
	N1 = 5
	if 1:
		shared_pairs = np.array(shared_eQTLs[shared_eQTLs[0] > N1].index)
		print('Shared eQTLs exist in %d tissues: %d' % (N1, len(shared_pairs)))

                outfn = '%s/%s_outlierPairs_group%d.txt' % (pairdir, LMfn, -1)
		pd.DataFrame(shared_pairs).to_csv(outfn, sep='\t', index = False, header=False)
