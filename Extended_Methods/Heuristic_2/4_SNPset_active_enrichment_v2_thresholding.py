...
def factorsActive(featureName, sub_feature = None):
	...
	mT, unmT = dict(), dict()
		...
		print(group)
                try:
                        outlier_fc = pd.read_csv('%s/%s_outlierSNPs_%s_group%d.txt' % (sigSNPfeaturedir, prefix, featureName, group), sep='\t', index_col = 0)
			outlier_fc.columns = [x.replace('chr1_','').replace('_Enh.srt.nullmatched', '') for x in outlier_fc.columns]
                except:
                        continue
		...
		outlier_rd = pd.read_csv('%s/%s_outlierSNPs_random_matched_%s_group%d_5folds.txt' % (sigSNPfeaturedir, prefix, featureName, group), sep='\t', index_col = 0)
		#outlier_rd = pd.read_csv('%s/%s_outlierSNPs_%s_group%d.txt' % (sigSNPfeaturedir, prefix, featureName, group), sep='\t', index_col = 0)
		outlier_rd.columns = [x.replace('chr1_','').replace('_Enh.srt.nullmatched', '') for x in outlier_rd.columns]


		## intersect with open regions
		if intersectWithOpen:
			try:
				outlier_fc_open = pd.read_csv('%s/%s_outlierSNPs_%s_group%d.txt' % (sigSNPfeaturedir, prefix, 'DNase', group), sep='\t', index_col = 0)
                        	outlier_fc_open.columns = [x.replace('chr1_','') for x in outlier_fc_open.columns]
			except:
				continue
			outlier_rd_open = pd.read_csv('%s/%s_outlierSNPs_random_matched_%s_group%d_5folds.txt' % (sigSNPfeaturedir, prefix, 'DNase', group), sep='\t', index_col = 0)
			#outlier_rd_open = pd.read_csv('%s/%s_outlierSNPs_%s_group%d.txt' % (sigSNPfeaturedir, prefix, 'DNase', group), sep='\t', index_col = 0)
		...
		for matched_tissue in existing_tissues:
			...
			#print(featureSNpset)
			#print(SNPset_size)
			#print(featureSNpset_rd)
			#print(SNPset_size_rd)
...
def wrap_up(featureName, sub):
	...
	if intersectWithOpen:
        	enrichment_results.to_csv('%s/%s_Active_enrichment_%s_%s_to%s_WithInOpen.txt' % (outdir, prefix, featureName, sub.replace('/','_'), background), index=False)
		tested_N.to_csv('%s/%s_Active_enrichment_N_%s_%s_to%s_WithInOpen.txt' % (outdir, prefix, featureName, sub.replace('/','_'), background), index=False)
	else:
                enrichment_results.to_csv('%s/%s_Active_enrichment_%s_%s_to%s.txt' % (outdir, prefix, featureName, sub.replace('/','_'), background), index=False)
                tested_N.to_csv('%s/%s_Active_enrichment_N_%s_%s_to%s.txt' % (outdir, prefix, featureName, sub.replace('/','_'), background), index=False)
...
if __name__ == '__main__':
	...
	#wrap_up('bedfiles_NullEnh_1X', 'None')
	for sub in ['7_Enh', '1_TssA']:
		wrap_up('ROADMAP', sub)
