...
def readin_outliers(theTis):
    ## outliers
    outLier = dict()

    outLier_df = pd.read_csv('%s/%s_ts_ciseQTL_closeToTop.txt' % (pairdir, theTis), sep='\t', header=None)
    outLier_df.columns = ['Gene', 'SNP']
    outLier[theTis] = outLier_df


    outLier_df = pd.DataFrame()
    for i in [idx,idx+50]:
	outLier_dfi = pd.read_csv('%s/match_random/batches_random/%s_ts_ciseQTL_closeToTop_random_matched_idx%d.txt' % (pairdir, theTis, i), sep='\t', header=None)
...
def match_SNPset_size_Gene(ref_pairs, other_pairs):
        ...
        dist1 = ref_pairs.groupby('Gene').size().sort_values(ascending = False)
        dist2 = other_pairs.groupby('Gene').size().sort_values(ascending = False)
...
def find_asscio_pairs_for_snps_in_TFBS_compare_rr(ts_pair, background_pair, TF_genes, TF_snps):
        ...
        A = np.sum(ts_gene_prop > prop)
        B = len(ts_gene_prop) - A
        C = np.sum(bg_gene_prop > prop)
...
def collect_results(pair_set1, pair_set2, tfi):
    ## collect results
    OR1, PV1, OR2, PV2, TF = [], [], [], [], []
    N1, N2, N3, N4, N5, N6 = [], [], [], [], [], []
    if 1:
...
if __name__ == '__main__':
    ...
    try:
	TF_allsnp_genes = np.load('%s/TFBS_allsnpgenes/TFBS_allsnpgenes_%s_%s_THR%d_Thresholding.npz' % (outdir, featureName, theTis, THR))
    except:
        sys.exit()
    TF_allSNP = TF_allsnp_genes['TFBS_snps'][()]
    TF_allGenes = TF_allsnp_genes['TFBS_genes'][()]

    tfi = np.sort(TF_allSNP.keys())[tfi_idx]
    ...
    if 1:
        ...
        background_pair = outLier[-1]

	#[ts_pair, background_pair] = separate_pairs(ts_pair, bg_pair, countAs='both')
	result = collect_results(ts_pair, background_pair, tfi)
	result.to_csv('%s/0815_noRestriction/perTF/TF_enrichment_%s_THR%d%s%s_TFBS_%s_torandom_idx%d_%s.txt' % (outdir, featureName, THR, randomPattern, theTis, LMfn, idx, tfi), sep='\t')
