...
if __name__ == '__main__':
	...
	SNP_TFBS = pd.read_csv('%s/%s_Active_SNPset_%s_TFmotif_%s.txt' % (activeSNPfeaturedir, FMfn, featureName, group), sep = '\t', index_col = 0)	
	if len(SNP_TFBS) == 0:
		sys.exit()
	...
	for tfi in SNP_TFBS.columns:
		...
		for chromosome in set(snps_df['SNP_chr']):
			...
			print(chromosome, len(np.unique(genes_inTSS)))
	...
	outfn = '%s/TFBS_allsnpgenes_%s_%s_THR%d_%s_new' % (outdir, featureName, group, THR, FMfn)
        #print(outfn)
