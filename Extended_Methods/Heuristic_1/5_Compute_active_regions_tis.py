...
def readInGenomicAnnotation_factors(active_regions, featureName, sub):
	...
	#for tis in ['Shared'] + tissues:
	for tis in ['Shared']:
		print(tis)
		if tis != 'Shared':
                	existing_tissues = np.intersect1d([tis], active_regions.columns)
		else:
			existing_tissues = np.intersect1d(tissues, active_regions.columns)
		...
		outfile = open('%s/%s_Active_SNPset_%s_%s_%s.txt' % (activeSNPdir, FMfn, featureName, sub.replace('/','_'), tis), 'w')
